...
| `normalize_functions` | How to normalize function name casing. `False` (default) preserves the casing of custom and audit function names as written; `"upper"` uppercases all function names; `"lower"` lowercases all function names. Note: SQLGlot built-in function names may still be canonicalized by the parser regardless of this setting. | string \| boolean |    N     |
