...
def test_get_expired_snapshots_scoped_to_target_ids(
    state_sync: EngineAdapterStateSync, make_snapshot: t.Callable
) -> None:
    """Test that get_expired_snapshots with target_snapshot_ids only returns snapshots in the target set."""
    now_ts = now_timestamp()
    ...
    # Both snapshots are expired (no active environments).
    # When scoped to only snapshot_a, only snapshot_a should be returned.
    batch = state_sync.get_expired_snapshots(
        ignore_ttl=True,
        batch_range=ExpiredBatchRange.all_batch_range(),
        target_snapshot_ids=[snapshot_a.snapshot_id],
    ...
    # snapshot_b should still exist because it was not in the target set.
    batch_all = state_sync.get_expired_snapshots(
        ignore_ttl=True,
        batch_range=ExpiredBatchRange.all_batch_range(),
    )
